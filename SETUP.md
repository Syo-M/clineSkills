...
- VS Code + **Cline v3.48以降**(Skills機能対応版)。`paths:`条件ルール・Workflows・Compact Promptも使うため、Clineも最新に保つこと
  - ※実機動作確認済み: **Cline 4.0.6**(2026-07、macOS)。Compact Prompt ON でスキル自動読込・ルール追従を確認(EVALUATION.md 実機検証)
...
- **Act(実装)モード**: **`qwen3-coder:30b`**(2026-07時点で36GB Macのコーディング用途の実質最適。実測で最も安定)。Qwen3-Coder-30B-**A3B(MoE、実効約3.3B)**、4-bitで約18-19GB、256Kネイティブ文脈。`ollama pull qwen3-coder:30b` → setup.sh または Modelfile で num_ctx を焼き込む。
  - ※「dense だから安定」ではない(これもMoE)。安定の理由は**専用ツールテンプレート**の質。
- **Plan(設計)モード**: `gemma4-cline`(相談・読み用途)。
- **試す価値のある代替**(必ず `packs/frontend/eval/golden-prompts.md` で採点してから採用):
  - **Devstral Small 2 24B**(Mistral、**dense**、約15GB、エージェント特化、文脈に余裕大)。ただしCline側で tool-use モードを「Mistral function calling」に設定しないと同種のパーサ・ループに当たる。
  - **Qwen3.6-27B**(dense、約17GB、2026-04公開、MLXタグあり)/ **GLM-4.7-Flash**(30B-A3B、Cline対応を明示)。いずれもCline×Mac実機の独立検証は薄いので要自己採点。
- **30B 未満(gemma4:12b 等)は編集用に非推奨**(ツール呼び出しの書式崩れが増える)。
- サンプリングは各モデルの**公式推奨値**をModelfileに焼き込み済み(Qwen: temperature 0.7 / top_p 0.8 / top_k 20 / repeat_penalty 1.05、Gemma: 1.0 / 0.95 / 64)。低temperatureは書式崩れを誘発するため避ける。
- **速度を上げたいなら MLX(LM Studio 経由)**: M4 では同じ qwen3-coder-30b で Ollama比 約3倍のスループット報告(~130 vs ~43 tok/s)。ただし**ツール呼び出しの不具合は直らない**(あれはCline側のパーサ問題)。速度改善策であって安定化策ではない。長文脈(30K+)では逆に遅くなる報告もあり要検証。
...
| `requires_approval` 等の必須パラメータ欠落でループ / ツール呼び出しの書式崩れ | 多くはモデルではなく**Cline側のツールパーサ不一致**(ローカルモデルはXMLテキスト経路にフォールバック — [Cline #10843](https://github.com/cline/cline/issues/10843))+ ツールテンプレート品質差。対処順: ①Compact Prompt を OFF(XML書式指示を復活)②num_ctx≥32K を確認 ③**Start New Task** でリセット ④それでも多発するモデルは避け、Cline実績のある **Qwen3-Coder-30B** に差し替え(同じMoEでも専用ツールテンプレートで安定) |
