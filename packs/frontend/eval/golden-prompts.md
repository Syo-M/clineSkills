...
※この課題はサーバー境界を前提とする。Next.jsなど**バックエンドを持つfixture**ではこのまま。純粋なVite SPAのfixtureでは「APIの置き場所」が曖昧でモデルが迷走しやすいので、置き場所だけ補って `Express でコメントを保存する API エンドポイントを追加して` とする(セキュリティ要件は伏せたまま=採点対象は変えない)。P6も同様にサーバー前提。
